...
class YOLOV5GradCAM:
    ...
    def forward(self, input_img, class_idx=True):
        """
        Args:
            input_img: input images with shape of (1, 3, H, W)
        Return:
...
class YOLOV5TorchObjectDetector(nn.Module):
    ...
    def non_max_suppression(prediction, logits, conf_thres=0.6, iou_thres=0.45, classes=None, agnostic=False,
        ...
        Returns:
             list of detections, on (n,6) tensor per images [xyxy, conf, cls] and pruned input logits (n, number-classes)
        """
        ...
        for xi, (x, log_) in enumerate(zip(prediction, logits)):  # images index, images inference
            # Apply constraints
    ...
    def forward(self, img):
        ...
        self.boxes, self.class_names, self.classes, self.confidences = [[[] for _ in range(img.shape[0])] for _ in
                                                                        range(4)]
        for i, det in enumerate(prediction):  # detections per images
            if len(det):
